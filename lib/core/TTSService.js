...
export class TTSService {
  ...
  async saveUploadRecords() {
    await fs.writeFile(
      ...
      JSON.stringify(this.uploadRecords, null, 2),
    );
  }
  ...
  generateFileHash(fileId, language, content) {
    ...
    return `${fileId}_${language}_${content.length}_${Buffer.from(start + end)
      .toString("base64")
      .substring(0, 10)}`;
  }
  ...
  truncateContentForTTS(content, maxBytes = 4800) {
    ...
    while (encoder.encode(truncated).length > maxBytes) {
      ...
      if (encoder.encode(truncated).length > maxBytes) {
        while (
          encoder.encode(truncated).length > maxBytes &&
          truncated.length > 0
        ) {
  ...
  async generateLongAudio(file, ttsClient, drive, ttsContent, voiceConfig) {
    ...
    Logger.warning(
      "⚠️  Content exceeds 5000 bytes. Truncating to fit standard TTS API...",
    );
    const truncatedContent = this.truncateContentForTTS(ttsContent);

    Logger.info(
      `📝 Truncated from ${Buffer.byteLength(ttsContent, "utf8")} to ${Buffer.byteLength(truncatedContent, "utf8")} bytes`,
    );
  ...
  async generateAndUpload(file, ttsClient, drive) {
    ...
    if (contentBytes > 5000) {
      Logger.warning(
        `⚠️  Content too long (${contentBytes} bytes). Using Long Audio API...`,
      );
      return await this.generateLongAudio(
        file,
        ttsClient,
        drive,
        ttsContent,
  ...
  async processAll() {
    ...
    Logger.gray(
      `Found ${Object.keys(this.uploadRecords).length} existing upload records`,
    );
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        Logger.error(
          `❌ Failed: ${file.language}/${file.id}: ${error.message}`,
        );
      }
    }
    ...
    Logger.success(
      `🎉 TTS processing completed! ${successCount}/${files.length} successful`,
    );
  }
}
