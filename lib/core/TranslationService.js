...
export class TranslationService {
  ...
  async translateContentWithGCP(sourceData, targetLang) {
    ...
    console.log(
      chalk.blue(`🌐 Translating from ${sourceLangCode} to ${targetLangCode}`),
    );
  ...
  async translateText(text, sourceLang, targetLang) {
    ...
    return await RetryUtils.retryOperation(operation, {
      ...
      onRetry: (error, attempt, maxRetries) => {
        console.log(
          chalk.yellow(
            `  🔄 Translation retry ${attempt}/${maxRetries}: ${error.message}`,
          ),
        );
      },
    });
  ...
  static async translateAll(targetLangs = null) {
    ...
    console.log(
      chalk.blue.bold("🌐 Multi-Language Translation (Google Cloud)"),
    );
      ...
      for (const lang of targetLangs) {
        if (missing.includes(lang)) {
          tasks.push({
            fileId: file.id,
            targetLang: lang,
            title: file.data.language[LANGUAGES.PRIMARY].title,
          });
        }
      }
    }
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        console.log(
          chalk.red(
            `❌ Failed: ${task.fileId} → ${task.targetLang}: ${error.message}`,
          ),
        );
      }
    }
    ...
    console.log(
      chalk.green.bold(
        `🎉 Translation completed! ${successCount}/${tasks.length} successful`,
      ),
    );
  }
