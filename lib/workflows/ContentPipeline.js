...
import { SocialMediaService } from "../core/SocialMediaService.js";
...
export class ContentPipeline {
  ...
  async handleInterrupt() {
    ...
    console.log(
      chalk.blue(
        "💾 Pipeline interrupted. Run again to continue from where files left off",
      ),
    );
    process.exit(0);
  }
  ...
  async runTranslationStep() {
    ...
    for (const file of files) {
      const missing = await ContentManager.getMissingTranslations(
        file.data,
        file.category,
      );
    ...
    await this.processFilesWithProgress(tasks, "Translation", async (task) => {
      ...
      const category = await this.getCategoryFromFileId(task.fileId);
      ...
      try {
        ...
        const TranslationServiceToUse = TranslationService;
        const result = await TranslationServiceToUse.translateFile(
          task.fileId,
          task.targetLang,
        );
        return result;
  ...
  async runSocialStep() {
    const files = await this.findFilesNeedingSocial();
    if (files.length === 0) {
      console.log(chalk.green("✅ No files need social media hook generation"));
      return;
    }
    ...
    if (!claudeAvailable) {
      console.log(chalk.yellow("⚠️  Claude command not found. Install with: npm install -g claude-code"));
      console.log(chalk.yellow("⚠️  Skipping social media hook generation"));
      return;
    }

    console.log(chalk.blue(`🚀 Using batch optimization for ${files.length} files`));
    
    // Use optimized batch processing instead of individual files
    const startTime = Date.now();
    const results = await SocialMediaService.processSocialFiles(files);
    ...
    console.log(chalk.green(`✅ Social hooks completed in ${duration}s:`));
    console.log(`  ${chalk.green("Generated:")} ${successful - cached}`);
    console.log(`  ${chalk.blue("Cached:")} ${cached}`);
    if (failed > 0) {
    ...
    this.progressBar = new cliProgress.SingleBar(
      {
        format:
          chalk.cyan("{bar}") +
          " {percentage}% | {value}/{total} | ETA: {eta}s | {stepName}",
        barCompleteChar: "\u2588",
        barIncompleteChar: "\u2591",
        hideCursor: true,
      },
  ...
  async findFilesNeedingSocial() {
    // Find translated files that need social media hook generation
    return await SocialMediaService.getFilesNeedingSocial();
  }
  ...
  async processTTSFile(file) {
    ...
    // Generate audio
    const audioResponse = await this.ttsService.synthesizeSpeech(
      ttsContent,
      voiceConfig,
    );
  ...
  async reset() {
    console.log(
      chalk.green(
        "✅ Pipeline reset (file-based state will automatically detect current status)",
      ),
    );
  }
  ...
  async status() {
    ...
    const socialFiles = await this.findFilesNeedingSocial();

    console.log(`Files needing translation: ${translationFiles.length}`);
    console.log(`Files needing TTS: ${ttsFiles.length}`);
    console.log(`Files needing social hooks: ${socialFiles.length}`);
  }
}
