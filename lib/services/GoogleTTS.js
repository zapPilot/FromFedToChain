...
export class GoogleTTSService {
  ...
  async synthesizeSpeech(text, voiceConfig) {
    ...
    if (textBytes > 4500) { // Leave some buffer
      console.log(chalk.yellow(`  ⚠️  Text too long (${textBytes} bytes), chunking...`));
  ...
  async synthesizeLongText(text, voiceConfig) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const chunkResponse = await RetryUtils.retryOperation(operation, {
        ...
        retryCondition: RetryUtils.isRetryableError,
        onRetry: (error, attempt, maxRetries) => {
          console.log(chalk.yellow(`  🔄 Chunk ${i + 1} retry ${attempt}/${maxRetries}: ${error.message}`));
        }
      });
  ...
  async mergeMP3Buffers(audioChunks) {
    ...
    console.log(chalk.gray(`  🎶 Merging ${audioChunks.length} LINEAR16 audio chunks...`));
    ...
    console.log(chalk.gray(`  ✅ Audio merge completed (${Math.round(wavBuffer.length / 1024)}KB WAV)`));
