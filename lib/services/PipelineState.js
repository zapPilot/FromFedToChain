...
export class PipelineStateManager {
  ...
  getProgressSummary() {
    ...
    steps.forEach(step => {
      const completed = Object.keys(this.state.completedFiles[step] || {}).length;
  ...
  printStatus() {
    ...
    if (summary.totalFiles > 0) {
      const progress = Math.round((summary.processedFiles / summary.totalFiles) * 100);
      console.log(chalk.yellow(`Progress: ${summary.processedFiles}/${summary.totalFiles} (${progress}%)`));
    }
  }
