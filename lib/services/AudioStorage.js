...
export class AudioStorageService {
  ...
  async saveAudioFile(audioBuffer, fileId, language, category, extension = 'wav') {
  ...
  async audioFileExists(fileId, language, category, extension = 'wav') {
    const audioPath = await this.getAudioPath(fileId, language, category, extension);
  ...
  async deleteAudioFile(fileId, language, category, extension = 'wav') {
    ...
    try {
      ...
      console.log(chalk.yellow(`  🗑️  Deleted audio: ${path.relative('.', filePath)}`));
      return true;
    } catch (error) {
  ...
  async listAudioFiles(language = null, category = null) {
    ...
    const categories = category ? [category] : ['daily-news', 'ethereum', 'macro'];
