...
describe("TranslationService Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const testContent = {
      ...
      title: "比特幣價格突破新高",
      content:
        "今天比特幣價格突破了歷史新高，這對加密貨幣市場來說是一個重要的里程碑。",
      references: ["測試來源1"],
      audio_file: null,
      social_hook: null,
      feedback: {
  ...
  describe("Google Cloud Translate Client Initialization", () => {
    it("should initialize translate client with service account", () => {
      ...
      assert.strictEqual(
        typeof TranslationService.getTranslateClient,
        "function",
      );
  ...
  describe("translateText Method", () => {
    it("should translate Chinese text to English", async () => {
      ...
      const [text, options] =
        mockTranslateClient.translate.mock.calls[0].arguments;
  ...
  describe("translate Method (Content Translation)", () => {
    it("should translate content to English successfully", async () => {
      mockTranslateClient.translate.mock.mockImplementation((text) => {
        if (text === "比特幣價格突破新高") {
          ...
          return Promise.resolve([
            "Today Bitcoin price broke the historical high, this is an important milestone for the cryptocurrency market.",
          ]);
        }
      ...
      const result = await TranslationService.translate(
        "2025-07-01-translation-test",
        "en-US",
      );

      assert.strictEqual(
        result.translatedTitle,
        "Bitcoin price breaks new high",
      );
      assert.strictEqual(
      ...
      const translationPath = path.join(
        ...
        "en-US",
        "daily-news",
        "2025-07-01-translation-test.json",
      );
    ...
    it("should translate content to Japanese successfully", async () => {
      mockTranslateClient.translate.mock.mockImplementation((text) => {
        if (text === "比特幣價格突破新高") {
          ...
          return Promise.resolve([
            "今日、ビットコインの価格が史上最高値を突破しました。これは暗号通貨市場にとって重要なマイルストーンです。",
          ]);
        }
        return Promise.resolve([text]);
      });
    ...
    it("should throw error if content is not reviewed", async () => {
      ...
      await assert.rejects(
        ...
        },
        {
          name: "Error",
          message:
            "Content must be reviewed before translation. Current status: draft",
        },
      );
    });
    ...
    it("should update source status to translated when all languages are complete", async () => {
      ...
      let sourceContent = await ContentManager.readSource(
        "2025-07-01-translation-test",
      );
      ...
      sourceContent = await ContentManager.readSource(
        "2025-07-01-translation-test",
      );
    ...
    it("should translate content to all supported languages", async () => {
      mockTranslateClient.translate.mock.mockImplementation((text, options) => {
        if (text === "比特幣價格突破新高") {
          ...
          if (options.to === "en") {
            return Promise.resolve([
              "Today Bitcoin price broke the historical high.",
            ]);
          } else if (options.to === "ja") {
            return Promise.resolve([
              "今日、ビットコインの価格が史上最高値を突破しました。",
            ]);
          }
        }
      ...
      const results = await TranslationService.translateAll(
        "2025-07-01-translation-test",
      );
      ...
      assert.strictEqual(
        results["ja-JP"].translatedTitle,
        "ビットコインの価格が新高値を突破",
      );
      assert.strictEqual(
        results["en-US"].translatedContent,
        "Today Bitcoin price broke the historical high.",
      );
      assert.strictEqual(
        results["ja-JP"].translatedContent,
        "今日、ビットコインの価格が史上最高値を突破しました。",
      );
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should handle content with missing title or content fields", async () => {
      ...
      const incompletePath = path.join(
        incompleteDir,
        `${incompleteContent.id}.json`,
      );
      await fs.writeFile(
        incompletePath,
        JSON.stringify(incompleteContent, null, 2),
      );
