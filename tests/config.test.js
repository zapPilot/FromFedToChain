...
describe("Configuration", () => {
  it("should have valid language configuration", () => {
    ...
    LANGUAGES.SUPPORTED.forEach((lang) => {
      ...
      assert(
        VOICE_CONFIG[lang].languageCode,
        `Missing languageCode for ${lang}`,
      );
  ...
  it("should support social media languages", () => {
    ...
    socialLanguages.forEach((lang) => {
      assert(
        LANGUAGES.SUPPORTED.includes(lang),
        `Social language ${lang} not in supported languages`,
      );
      assert(
        VOICE_CONFIG[lang],
        `Social language ${lang} missing voice config`,
      );
    });
  });
});
