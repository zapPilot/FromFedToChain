...
describe("Audio Architecture Tests", () => {
  ...
  async function createTestContent() {
    const sourceContent = {
      ...
      title: "測試音頻內容標題",
      content:
        "這是用於測試音頻生成的中文內容。它包含足夠的文字來測試TTS功能。",
      references: ["測試來源"],
      audio_file: null,
      social_hook: null,
      feedback: {
    ...
    const enContent = {
      ...
      content:
        "This is English content for testing audio generation. It contains enough text to test TTS functionality.",
      status: "translated",
    };
    ...
    const jaContent = {
      ...
      content:
        "これは音声生成をテストするための日本語コンテンツです。TTS機能をテストするのに十分なテキストが含まれています。",
      status: "translated",
    };
  ...
  describe("Unified Language Configuration", () => {
    it("should include zh-TW in audio languages", () => {
      ...
      assert(
        audioLangs.includes("zh-TW"),
        "zh-TW should be included in audio languages",
      );
      assert(
        audioLangs.includes("en-US"),
        "en-US should be included in audio languages",
      );
      assert(
        audioLangs.includes("ja-JP"),
        "ja-JP should be included in audio languages",
      );
    });
  ...
  describe("Audio Generation with zh-TW Inclusion", () => {
    beforeEach(() => {
      ...
      const originalPrepareContent =
        AudioService.constructor.prototype.prepareContentForTTS;
      ...
      AudioService.generateAudio = async function (id, language) {
        ...
        const audioResponse = await mockTTSService.synthesizeSpeech(
          content.content,
          voiceConfig,
        );
    ...
    it("should verify TTS configuration is passed correctly", async () => {
      ...
      const callArgs =
        mockTTSService.synthesizeSpeech.mock.calls[0].arguments[1];
  ...
  describe("Folder Structure Consistency", () => {
    ...
    it("should handle different categories correctly", async () => {
      ...
      assert(
        audioPath.includes("zh-TW/macro/"),
        "Should create macro category structure",
      );
    });
