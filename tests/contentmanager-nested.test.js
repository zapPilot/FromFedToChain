...
describe('ContentManager Nested Structure Tests', () => {
  ...
  describe('Folder Structure Validation', () => {
    ...
    it('should read content by ID and specific language', async () => {
      const content = await ContentManager.read('2025-07-01-test-content', 'zh-TW');
      assert.equal(content.title, '測試文章標題');
      assert.equal(content.language, 'zh-TW');
    });
  ...
  describe('Source Content Operations', () => {
    ...
    it('should create source content correctly', async () => {
      ...
      const filePath = path.join(tempDir, 'zh-TW', 'ethereum', '2025-07-01-new-test.json');
      const fileExists = await fs.access(filePath).then(() => true).catch(() => false);
      assert(fileExists, 'File should exist in zh-TW/ethereum/');
    });

    it('should update source content status', async () => {
      await ContentManager.updateSourceStatus('2025-07-01-test-content', 'reviewed');
      
      const content = await ContentManager.readSource('2025-07-01-test-content');
  ...
  describe('Translation Operations', () => {
    it('should create translation file in correct nested structure', async () => {
      ...
      // Verify file was created in correct location
      const filePath = path.join(tempDir, 'en-US', 'daily-news', '2025-07-01-test-content.json');
      const fileExists = await fs.access(filePath).then(() => true).catch(() => false);
    ...
    it('should list available languages for content ID', async () => {
      ...
      const languages = await ContentManager.getAvailableLanguages('2025-07-01-test-content');
    ...
    it('should get all language versions of content', async () => {
      ...
      const allVersions = await ContentManager.getAllLanguagesForId('2025-07-01-test-content');
  ...
  describe('Audio and Social Hook Operations', () => {
    ...
    it('should add audio file to specific language', async () => {
      ...
      await ContentManager.addAudio('2025-07-01-test-content', 'en-US', audioPath);
      
      const content = await ContentManager.read('2025-07-01-test-content', 'en-US');
    ...
    it('should add social hook to specific language', async () => {
      ...
      await ContentManager.addSocialHook('2025-07-01-test-content', 'en-US', hook);
  ...
  describe('Feedback Operations', () => {
    ...
    it('should add AI feedback to specific language file', async () => {
      ...
      assert.equal(content.feedback.ai_outputs.translation.comments, 'Excellent translation');
    });
  });
  ...
  describe('Performance and Edge Cases', () => {
    ...
    it('should prevent access outside nested structure', async () => {
      try {
        await ContentManager._readFromLanguage('2025-07-01-test-content', 'invalid-language');
        assert.fail('Should have thrown an error');
      } catch (error) {
        assert(error.message.includes('Content not found'));
      }
