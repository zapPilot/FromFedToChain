...
describe("Sanity Check Tests", () => {
  ...
  describe("Content Schema Validation", () => {
    ...
    it("should detect missing required fields", () => {
      ...
      const requiredFields = ['id', 'status', 'category', 'language', 'title', 'content', 'feedback'];
      
      requiredFields.forEach(field => {
        const invalidContent = { ...validContent };
        delete invalidContent[field];
        
        assert.throws(() => ContentSchema.validate(invalidContent), 
  ...
  describe("File System Structure", () => {
    ...
    it("should have services directory with service files", async () => {
      ...
      const serviceFiles = ['TranslationService.js', 'AudioService.js', 'SocialService.js'];
      ...
      try {
        ...
        for (const language of languages) {
          ...
          try {
            ...
            assert(langStats.isDirectory(), `${language} directory should exist`);
          } catch (e) {
        ...
        console.log("Note: Content directory doesn't exist in test environment");
      }
    });
  ...
  describe("Package Configuration", () => {
    ...
    it("should have working npm scripts defined", async () => {
      ...
      requiredScripts.forEach(script => {
        assert(packageObj.scripts[script], `Script ${script} should be defined`);
        assert(typeof packageObj.scripts[script] === 'string', `Script ${script} should be a string`);
      });
    });
  });
  ...
  describe("Content ID Format Validation", () => {
    it("should validate standard date-based ID format", () => {
      ...
      validIds.forEach(id => {
        assert.doesNotThrow(() => {
          ContentSchema.createContent(id, 'daily-news', 'zh-TW', 'Title', 'Content');
        }, `Should accept ID: ${id}`);
      });
    });
    ...
    it("should handle various content lengths", () => {
      ...
      assert.doesNotThrow(() => {
        const short = ContentSchema.createContent(
          '2025-07-13-short', 'daily-news', 'zh-TW', 'Title', shortContent
        );
      ...
      assert.doesNotThrow(() => {
        const long = ContentSchema.createContent(
          '2025-07-13-long', 'daily-news', 'zh-TW', 'Title', longContent
        );
    ...
    it("should handle empty and populated references", () => {
      assert.doesNotThrow(() => {
        const noRefs = ContentSchema.createContent(
          '2025-07-13-no-refs', 'daily-news', 'zh-TW', 'Title', 'Content'
        );
      ...
      assert.doesNotThrow(() => {
        const withRefs = ContentSchema.createContent(
          '2025-07-13-with-refs', 'daily-news', 'zh-TW', 'Title', 'Content',
