...
describe("Pipeline Tests", () => {
  ...
  async function createTestContent() {
    ...
    const socialContentEn = {
      ...
      social_hook:
        "🚀 Social Content Title - An exciting update! #crypto #news",
    };
  ...
  describe("getAllPendingContent Function", () => {
    ...
    it("should sort content by date (newest first)", async () => {
      ...
      const olderContent = {
        ...
        social_hook: null,
        feedback: {
          content_review: { status: "accepted" },
          ai_outputs: {},
          performance_metrics: {},
        },
        updated_at: new Date().toISOString(),
      ...
      assert(
        pendingContent[0].content.date >=
          pendingContent[pendingContent.length - 1].content.date,
      );
    });
    ...
    it.skip("should correctly identify current status of content", async () => {
      ...
      const reviewedItem = pendingContent.find(
        (item) => item.content.id === "2025-07-01-reviewed-content",
      );
      ...
      const audioItem = pendingContent.find(
        (item) => item.content.id === "2025-07-01-audio-content",
      );
      ...
      const socialItem = pendingContent.find(
        (item) => item.content.id === "2025-07-01-social-content",
      );
      assert.strictEqual(
        socialItem,
        undefined,
        "Social content should not be in pending list",
      );
    });
  });
  ...
  describe("runPipelineForContent Function", () => {
    ...
    it("should continue from audio status to social phase", async () => {
      ...
      try {
        ...
        const updatedContent = await ContentManager.readSource(
          "2025-07-01-audio-content",
        );
    ...
    it("should not process content that is not ready for next phase", async () => {
      ...
      await ContentManager.updateSourceStatus(
        "2025-07-01-reviewed-content",
        "draft",
      );

      const results = await runPipelineForContent(
        "2025-07-01-reviewed-content",
      );
    ...
    it.skip("should skip phases that are already completed", async () => {
      ...
      assert.strictEqual(
        ...
        0,
        "No phases should run for content already at final stage",
      );
    });
  ...
  describe("Pipeline Status Transition Bug Prevention", () => {
    it("should update status after successful pipeline step execution", async () => {
      ...
      try {
        ...
        const { ContentPipelineService } = await import(
          "../src/services/ContentPipelineService.js"
        );
        const success = await ContentPipelineService.processContentNextStep(
          "test-pipeline-status-fix",
        );
        ...
        assert.notStrictEqual(
          ...
          "translated",
          "Status must not remain stuck at translated",
        );
        assert.strictEqual(
          updatedContent.status,
          "wav",
          "Status should advance to wav",
        );
      } finally {
        AudioService.generateWavOnly = originalGenerateWavOnly;
      }
