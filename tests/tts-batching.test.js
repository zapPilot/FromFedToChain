  ...
  describe("Content Chunking", () => {
    ...
    it("should split large content into multiple chunks", () => {
      ...
      chunks.forEach((chunk, index) => {
        ...
        assert(
          chunkBytes <= 4800,
          `Chunk ${index} should be under 4800 bytes, got ${chunkBytes}`,
        );
      });
    });
    ...
    it("should split very long paragraphs by sentences", () => {
      ...
      const longParagraph = Array(90)
        .fill("This is a sentence that forms part of a very long paragraph. ")
        .join("");
    ...
    it("should handle content with mixed paragraph sizes", () => {
      ...
      chunks.forEach((chunk, index) => {
        ...
        assert(
          chunkBytes <= 4800,
          `Chunk ${index} exceeds byte limit: ${chunkBytes}`,
        );
      });
    });
    ...
    it("should handle edge case of extremely long single sentence", () => {
      ...
      const extremelyLongSentence =
        "This is an extremely long sentence that just keeps going and going without any periods or breaks " +
        "and continues for a very long time ".repeat(200) +
        "and finally ends.";
      ...
      assert(
        hasEllipsis,
        "Force-split chunks should include ellipsis for continuation",
      );
    });
  ...
  describe("Batched TTS Processing", () => {
    it("should process multiple chunks and combine audio", async () => {
      ...
      mockClient.synthesizeSpeech.mock.mockImplementation(() => {
        ...
        switch (callCount) {
          case 1:
            return Promise.resolve([{ audioContent: audioChunk1 }]);
          case 2:
            return Promise.resolve([{ audioContent: audioChunk2 }]);
          case 3:
            return Promise.resolve([{ audioContent: audioChunk3 }]);
          default:
      ...
      const largeContent = Array(300)
        .fill("This is a paragraph that will be split. ")
        .join("");
      const voiceConfig = { languageCode: "en-US", name: "en-US-Wavenet-D" };
    ...
    it("should add delays between chunk requests", async () => {
      ...
      const largeContent = Array(40)
        .fill("This is content that needs batching. ")
        .join("");
      const voiceConfig = { languageCode: "en-US", name: "en-US-Wavenet-D" };
      ...
      if (timestamps.length > 1) {
        for (let i = 1; i < timestamps.length; i++) {
          ...
          assert(
            delay >= 450,
            `Delay between calls should be at least 450ms, got ${delay}ms`,
          );
        }
      }
    });
    ...
    it("should handle batching errors gracefully", async () => {
      ...
      const largeContent = Array(200)
        .fill("Content that will cause an error. ")
        .join("");
      const voiceConfig = { languageCode: "en-US", name: "en-US-Wavenet-D" };
  ...
  describe("Audio Combination", () => {
    it("should correctly combine WAV audio chunks", () => {
      ...
      const expectedLength =
        chunk1.length + (chunk2.length - 44) + (chunk3.length - 44);
...
Visit our [website](https://crypto.com) for more details.
      ...
      const preparedContent = GoogleTTSService.prepareContentForTTS(
        rawContent,
        "en-US",
      );
      const voiceConfig = { languageCode: "en-US", name: "en-US-Wavenet-D" };
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle TTS service errors during batching", async () => {
      ...
      const largeContent = Array(50)
        .fill("Content that will cause API error. ")
        .join("");
      const voiceConfig = { languageCode: "en-US", name: "en-US-Wavenet-D" };
    ...
    it("should handle malformed audio chunks", () => {
      ...
      const combined = ttsService.combineAudioChunks([
        validChunk,
        invalidChunk,
      ]);
    ...
    it("should handle extremely large content", () => {
      ...
      const extremeContent = Array(1000)
        .fill("This is repeated content. ")
        .join("");
      const chunks = ttsService.splitContentIntoChunks(extremeContent);

      assert(
        chunks.length > 1,
        "Should create multiple chunks for extreme content",
      );
      assert(
        chunks.length >= 5,
        "Should create at least 5 chunks for 26KB content",
      );
