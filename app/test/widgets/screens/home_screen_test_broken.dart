...
import 'package:from_fed_to_chain_app/widgets/search_bar.dart' as search_bar;
...
class HomeScreenTestUtils {
  ...
  static void setupAuthServiceMocks(
    Map<String, dynamic> mockAuthService, {
    Map<String, dynamic>? currentUser,
    String authState = 'authenticated',
  }) {
    // Mock auth service setup - simplified for testing
    // In real implementation, this would mock proper AuthService
  }
  ...
  /// Pump widget and settle animations with timeout protection
  static Future<void> pumpAndSettle(
    ...
    // Use controlled pumps instead of pumpAndSettle to avoid timeouts
    await tester.pump(); // Initial frame
    await tester.pump(const Duration(milliseconds: 100)); // Allow animations
    
    try {
      await tester.pumpAndSettle(const Duration(milliseconds: 200));
    } catch (e) {
      // If timeout, do a few more controlled pumps
      for (int i = 0; i < 3; i++) {
  ...
  group('HomeScreen Widget Tests', () {
    ...
    group('Basic Rendering', () {
      testWidgets('renders HomeScreen with main components', (tester) async {
        ...
        // expect(find.byIcon(Icons.account_circle), findsOneWidget); // Auth disabled
    ...
    group('Search Functionality', () {
      testWidgets('shows search bar when search icon is tapped',
        ...
        expect(find.byType(search_bar.SearchBarWidget), findsNothing);
        expect(find.byIcon(Icons.search), findsOneWidget);
        ...
        expect(find.byType(search_bar.SearchBarWidget), findsOneWidget);
