      ...
      for (const item of items) {
        ...
        if (item.isDirectory()) {
          ...
          try {
            ...
            console.log(
              chalk.yellow(`⚠️  Warning: Could not parse ${fullPath}`),
            );
          }
        }
      }
...
async function main() {
  ...
  try {
    ...
    if (!fileId) {
      ...
      if (files.length === 0) {
        console.log(
          chalk.yellow("⚠️  No English content found. Run translation first."),
        );
        return;
      }
    ...
    console.log(
      chalk.green(`📄 Processing: ${targetFile.data.languages.en.title}`),
    );
