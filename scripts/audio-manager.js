    ...
    switch (command) {
      ...
      case "delete":
        if (args.length < 4) {
          console.log(
            chalk.red(
              "Usage: npm run audio delete <fileId> <language> <category>",
            ),
          );
          return;
        }
      ...
      default:
        ...
        console.log(
          chalk.cyan("npm run audio list") +
            chalk.gray(" - List all audio files"),
        );
        console.log(
          chalk.cyan("npm run audio list en-US") +
            chalk.gray(" - List files for language"),
        );
        console.log(
          chalk.cyan("npm run audio list en-US daily-news") +
            chalk.gray(" - List files for language+category"),
        );
        console.log(
          chalk.cyan("npm run audio stats") +
            chalk.gray(" - Show storage statistics"),
        );
        console.log(
          chalk.cyan("npm run audio delete <fileId> <lang> <category>") +
            chalk.gray(" - Delete audio file"),
        );
        break;
    }
  } catch (error) {
...
async function listAudioFiles(audioStorage, language, category) {
  ...
  files.forEach((file) => {
    ...
    console.log(
      `${chalk.cyan(file.fileId)} (${chalk.yellow(file.language)}) [${chalk.green(file.category)}]`,
    );
...
async function showStats(audioStorage) {
  ...
  Object.entries(stats.byLanguage).forEach(([lang, data]) => {
    console.log(
      `  ${lang}: ${data.count} files (${audioStorage.formatFileSize(data.size)})`,
    );
  });
  console.log("");
  ...
  Object.entries(stats.byCategory).forEach(([cat, data]) => {
    console.log(
      `  ${cat}: ${data.count} files (${audioStorage.formatFileSize(data.size)})`,
    );
  });
  console.log("");
  ...
  Object.entries(stats.byExtension).forEach(([ext, data]) => {
    console.log(
      `  ${ext.toUpperCase()}: ${data.count} files (${audioStorage.formatFileSize(data.size)})`,
    );
  });
}
...
async function deleteAudio(audioStorage, fileId, language, category) {
  console.log(
    chalk.yellow(`🗑️  Deleting audio: ${fileId} (${language}/${category})`),
  );

  const deleted = await audioStorage.deleteAudioFile(
    fileId,
    language,
    category,
  );
