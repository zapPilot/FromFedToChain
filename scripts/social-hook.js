...
function generateHookVariations(title, content, insights) {
  ...
  variations.push(
    `What if I told you: ${insights[0] || "crypto is changing finance"}?`,
  );
  ...
  variations.push(
    `Here's what everyone is missing about ${extractMainTopic(title)}...`,
  );
  variations.push(
    `The truth about ${extractMainTopic(title)} that nobody talks about`,
  );

  // Contrarian hooks
  variations.push(
    `Everyone thinks ${extractMainTopic(title)} is risky, but...`,
  );
...
function selectBestHook(variations, content) {
  ...
  const scored = variations.map((hook) => {
    ...
    if (
      /\d+/.test(hook) ||
      hook.includes("Bitcoin") ||
      hook.includes("crypto")
    ) {
      score += 2;
    }
...
async function main() {
  ...
  if (!fileId) {
    ...
    console.log(
      chalk.gray(
        "Usage: node scripts/social-hook.js <file_id> [--platform=twitter|linkedin|facebook]",
      ),
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      chalk.green(`✅ English content loaded: ${path.basename(filePath)}`),
    );
